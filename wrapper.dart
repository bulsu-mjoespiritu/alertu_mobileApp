...
class _WrapperState extends ConsumerState<Wrapper> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    // Keyboard fix: this Scaffold wraps the entire app (Homepage included)
    // and never set resizeToAvoidBottomInset, so it defaulted to true --
    // every keyboard open resized this WHOLE tree upward, dragging the map
    // screen's nav bar along with it. Homepage's own
    // resizeToAvoidBottomInset: false + MediaQuery.removeViewInsets could
    // only clean up the inset AFTER this outer Scaffold had already resized
    // for it, so it was fighting its parent every frame -- the stutter.
    //
    // False here stops the resize at the source. Screens that DO want the
    // normal push-up behavior (report submission, signup, complete profile,
    // forgot password, profile management) are pushed as their own routes
    // with their own Scaffold(resizeToAvoidBottomInset: true), which is
    // unaffected by this -- each route's own Scaffold is what resizes it.
    return Scaffold(
